...
function desiredFiles(root, profiles, agents) {
  ...
  for (const entry of entries) {
    ...
    output.set(entry.path, read(join(ASSET_ROOT, entry.path)));
  }
...
function audit(root) {
  ...
  while (stack.length) {
    ...
    for (const name of readdirSync(current)) {
      if (['.git', 'node_modules', 'dist', 'build', 'coverage'].includes(name)) continue;
      const path = join(current, name);
      if (statSync(path).isDirectory()) stack.push(path);
      else allFiles.push(repoPath(root, path));
    }
  }
...
function applyPlan(value) {
  ...
  try {
    ...
    const managedFiles = {};
    for (const action of value.actions) {
      if (action.path === CONFIG) continue;
